...
var P = .01, Id = 20, Ld = 13, Rd = /* @__PURE__ */ new Map([
	["Primary", 0],
	["Close", 1],
	["Near", 2],
	["Far", 3],
...
function Ud(e) {
	return [...e].sort((e, t) => {
		let n = (Rd.get(e.star.orbitClass) ?? 99) - (Rd.get(t.star.orbitClass) ?? 99);
...
function Yd(e, t, n) {
	let r = Ud(e).map((e) => ({
		...e,
		hzco: Jd(e, t),
		totalStarOrbits: qd(e.allowable.availableOrbitSpan, e.allowable.parentStarIds.length > 1),
		allocatedWorlds: 0,
		emptySlots: 0,
		spread: 0
...
function Xd(e, t) {
	...
	return Vd(e, n) && (r -= 2), [
		...
		"Ib",
		"II"
	].includes(n.luminosityClass) ? r += 3 : n.luminosityClass === "III" ? r += 2 : n.luminosityClass === "IV" ? r += 1 : n.luminosityClass === "VI" && --r, Hd(n) && (r -= 2), t < 6 ? r -= 4 : t <= 9 ? r -= 3 : t <= 12 ? r -= 2 : t <= 15 ? --r : t <= 17 ? r += 0 : t <= 20 ? r += 1 : r += 2, r -= e.filter((e) => [
		"Close",
		"Near",
		"Far"
...
function Zd(e, t, n, r) {
	...
	if (n.some((e) => e.orbitClass === "Close") && !Gd(i.allowable, i.hzco)) {
		let t = Kd(i.allowable, i.allowable.minimumAllowableOrbit, i.hzco), n = Math.floor(t / P), r = Math.min(i.allocatedWorlds, n);
		if (r <= 0) return {
			number: 0,
			mode: "cold",
			innerSplitCount: 0
...
function $d(e, t, n, r) {
	...
	if (r.mode === "split") {
		...
		return e ? O(Math.min(e.outer, e.inner + Math.abs(s)), 2) : Qd(t.allowable, i, s) ?? a;
	}
	if (r.mode === "cold") {
	...
	if (r.mode === "hot") {
		...
		return c < 0 && (c = Math.max(i - .1, a + t.allocatedWorlds * .01)), Qd(t.allowable, Math.min(i, c), s) ?? a;
	}
	return Qd(t.allowable, i + s, s) ?? a;
}
function ef(e) {
...
function rf(e, t, n, r) {
	...
	for (let t of e) {
		...
		let e = t.allocatedWorlds + t.emptySlots, n = nf(t.allowable.minimumAllowableOrbit, t.allowable.outerLimit ?? t.allowable.minimumAllowableOrbit, e);
...
function af(e, t, n) {
	...
	let r = e.ranges.findIndex((e) => Wd(e, t)), i = t;
	if (r < 0) {
...
function of(e, t, n, r, i) {
	let a = [], o = r;
	for (let r = 0; r < n; r += 1) {
		let n = e.roll(2, 6, -7).total * t.spread / 10, s = Math.max(P, t.spread + n), c = af(t.allowable, o, s);
		if (c === null) break;
		a.push({
			key: `regular-${i + r}`,
			zone: t,
...
function cf(e, t, n, r) {
	...
	for (let s of t) {
		...
		let c;
		if (s.star.orbitClass !== "Primary") c = of(e, s, t, s.allowable.minimumAllowableOrbit, a);
		else if (n.mode === "cold") c = of(e, s, t, Math.max(r - s.spread, s.allowable.minimumAllowableOrbit), a);
		else if (n.mode === "hot") c = sf(e, s, t, r + s.spread, a);
		else if (n.mode === "split") {
			let i = of(e, s, Math.min(t, n.innerSplitCount), s.allowable.minimumAllowableOrbit, a), o = t - i.length, l = o > 0 ? of(e, s, o, Math.max(r - s.spread, s.allowable.minimumAllowableOrbit), a + i.length) : [];
			c = [...i, ...l];
		} else c = of(e, s, t, s.allowable.minimumAllowableOrbit, a);
...
function df(e, t) {
	let n = Math.min(t, Math.max(0, Ld - e.terrestrialPlanets));
	return {
...
function ff(e, t, n) {
	let r = Math.max(e.allowable.minimumAllowableOrbit, Math.min(Id, t)), i = (t) => n.some((n) => n.zone.star.id === e.star.id && Math.abs(n.orbitNumber - t) < P / 2), a = Qd(e.allowable, r, P);
	if (a !== null && !i(a)) return a;
	for (let t = 1; t <= 600; t += 1) for (let n of [O(r + t * P, 2), O(r - t * P, 2)]) if (n >= e.allowable.minimumAllowableOrbit && n <= Id && Gd(e.allowable, n) && !i(n)) return n;
	return null;
}
function pf(e, t, n, r, i) {
	...
	for (let n = 0; n < i; n += 1) {
		...
		let l = uf(e.roll(2, 6).total), u = r.terrestrialPlanets + s < Ld ? "Terrestrial Planet" : "Planetoid Belt";
		if (u === "Terrestrial Planet" && (s += 1), l === "Trojan") {
			let n = t.filter((e) => !e.anomaly && e.worldKind === void 0);
			if (n.length > 0) {
				let r = n[zd(e, n.length)];
		...
		let d = a[zd(e, a.length)], f = ff(d, e.roll(2, 6, -2).total + e.d10ZeroToNine() / 10, t);
		if (f === null) {
			o += 1;
			continue;
		}
...
function mf(e) {
	return [...e].sort((e, t) => {
		let n = (Rd.get(e.zone.star.orbitClass) ?? 99) - (Rd.get(t.zone.star.orbitClass) ?? 99);
		if (n !== 0) return n;
...
function hf(e, t, n, r, i = () => !0) {
	let a = 0;
	for (let o = 0; o < r && t.length !== 0; o += 1) {
		let r = zd(e, t.length), o;
...
function gf(e, t, n, r) {
	let i = mf(t), a = 0, o = hf(e, i, "Empty Orbit", r, (e) => !e.anomaly && !e.protectedFromEmpty);
	a += r - o;
	let s = hf(e, i, "Gas Giant", n.gasGiants);
	a += n.gasGiants - s;
	let c = hf(e, i, "Planetoid Belt", n.planetoidBelts);
...
function bf(e, t, n, r, i) {
	let a = Yd(t, n, r.totalWorlds), o = Zd(e, a, n, r.totalWorlds), s = a.find((e) => e.star.orbitClass === "Primary") ?? a[0], c = s ? $d(e, s, r.totalWorlds, o) : 0, l = ef(e.roll(2, 6).total), u = tf(a, l), d = rf(a, n, o.number, c), f = cf(e, a, o, c), p = lf(e.roll(2, 6).total), m = df(r, p), h = pf(e, f.slots, a, r, p), g = gf(e, f.slots, r, l - u);
	return vf(e, f.slots, i), {
		worlds: yf(f.slots),
		counts: m,
		emptyOrbits: l - u,
		anomalousOrbits: p,
	...
	return t.map((t) => {
		let r = Gu(e, t, n[t.id]), i = t.physical?.details?.satellites, a = i && {
			...
			moons: i.moons.map((e, r) => {
				...
				let i = e.sourceId ?? `${t.id}.moon-${r + 1}`, a = new S(Cf(`wbh-moon-social-v1|${t.id}|${e.sourceId ?? r}`));
				return {
					...e,
					social: Gu(a, wf(t, e, r), n[i])
				};
			})
		};
		return {
...
function kf(e = {}) {
	let t = xf(e.settings), n = e.seed ?? Date.now(), r = new S(Ef(n, e.rerollIndex ?? 0)), i = e.name ?? "Uncharted System", a = dr(r, `${i} A`, "A", !0, t), o = Sf(r, i, a, t), s = o.stars.sort((e, t) => (e.orbitAu ?? 0) - (t.orbitAu ?? 0)), c = s.find((e) => e.id === a.id) ?? a, l = Vr(r, s), u = Fd(s), d = bf(r, s.filter((e) => e.orbitClass !== "Companion").map((e) => ({
		star: e,
	...
	} : p, g = h.selectedMainworldId ?? void 0, _ = pd(f, h.selectedMainworldId), v = Df(_, g, t, e.habitationOverrides), y = Tf(r, _, v), b = md(y, h.selectedMainworldId), x = b?.moon ? wf(b.world, b.moon, b.world.physical?.details?.satellites?.moons.indexOf(b.moon) ?? 0) : b?.world, ee = x?.social?.uwp ?? (x?.physical?.uwpPhysical ? `X${x.physical.uwpPhysical}000-0` : void 0), te = {
		...
		summary: {
			...
			forbiddenZones: u.primaryExclusions.map(Of),
...
var Af = /^([ABCDEX])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])-([0-9A-HJ-NP-Z])$/i;
function jf(e) {
	let t = e.trim().toUpperCase().replace(/\s+/g, "").match(Af);
...
function Pf(e, t) {
	...
	let n = Nf(e, t), r = e.d10ZeroToNine();
	return {
		method: "WBH population phase 1",
		populationCode: t,
		pValue: n,
		additionalSignificantDigit: r,
		estimatedPopulation: Mf(t, n, r),
...
function zf(e) {
	let t = jf(e.sourceUwp), n = kf(e), r = e.seed ?? Date.now(), i = Lf(n, new S(Ff(r, (e.rerollIndex ?? 0) + 101))), a = jl(i, n.primary, t.size, t.atmosphere, t.hydrographics), o = Pf(new S(If(`wbh-continuation-population-v1|${r}|${e.rerollIndex ?? 0}|${t.normalized}`)), t.population), s = {
		...
		...e,
		isMainworld: !1
	})).sort((e, t) => e.au - t.au).map(Rf), l = ps([{
		...i,
		physical: a,
		social: s
	}, ...c].sort((e, t) => e.au - t.au || (e.isMainworld ? -1 : 1)), n.stars).map((t) => {
		if (!t.isMainworld || !t.social) return t;
		let n = bu(new S(If(`wbh-social-pcr-v1|${r}|${e.rerollIndex ?? 0}|${t.id}|${t.social.uwp}`)), t, t.social), i = {
			...t.social,
			populationConcentration: n
		}, a = Eu(new S(If(`wbh-social-urbanisation-v1|${r}|${e.rerollIndex ?? 0}|${t.id}|${t.social.uwp}`)), t, i), o = {
			...i,
			urbanisation: a
		}, s = new S(If(`wbh-social-major-cities-v1|${r}|${e.rerollIndex ?? 0}|${t.id}|${t.social.uwp}`));
		return {
			...t,
			social: {
				...o,
...
function Bf(e) {
	return An(tn(Wt(Hn(xn(xe(pe(bt(Le(De(ln(kt(qe(nt(zf(e)))))))))))))));
}
//#endregion
...
function Hf(e) {
	...
	return [
		...
		1,
		10
	].includes(n) ? s(8, `WBH Atmosphere ${Vf(n)}`) : [
		2,
		3,
		13,
		14
	].includes(n) ? s(5, `WBH Atmosphere ${Vf(n)}`) : [
		4,
		7,
		9
	].includes(n) ? s(3, `WBH Atmosphere ${Vf(n)}`) : n === 11 ? s(9, "WBH Atmosphere B") : n === 12 ? s(10, "WBH Atmosphere C") : n === 15 ? s(8, "WBH Atmosphere F conservative floor") : (n === 16 || n === 17) && s(14, `WBH Atmosphere ${Vf(n)}`), typeof i == "number" && (i === 0 ? s(8, "WBH Habitability 0") : i <= 2 ? s(5, `WBH Habitability ${i}`) : i <= 7 && s(3, `WBH Habitability ${i}`)), r === 0 && n >= 4 && s(4, "Existing dry-world survival floor"), t === 0 && s(8, "Existing Size 0 survival floor"), {
		minimum: a,
		basis: o
	};
}
...
function Gf(e, t, n, r, i, a, o) {
	...
	let s = Hf(e), c = typeof e.social?.populationCode == "number" ? e.social.populationCode : null, l = e.physical.details?.nativeLife?.currentNativeSophont === !0, u = r >= s.minimum;
	return {
		id: e.id,
		parentId: t,
...
function Yf(e) {
	...
	if (t) for (let n of e.worlds) {
		...
		if (r >= 0) return Jf(n, e[r], r);
	}
}
function Xf(e = {}) {
	let t = An(tn(Wt(Hn(xn(xe(pe(bt(Le(De(ln(kt(qe(nt(kf(e))))))))))))))), n = Yf(t), r = t.summary.mainworldId ?? "none", i = n?.social?.uwp ?? "none", a = Kf(new S(qf(`wbh-secondary-populations-v1|${t.id}|${r}|${i}`)), t.worlds, n);
	return {
		...t,
...
var Zf = "traveller-system-generator/orbital-state-v1", Qf = "tsg-system-epoch-v1", $f = 149597870.7, ep = 365.25 * 86400;
function tp(e) {
	let t = 2166136261;
	for (let n = 0; n < e.length; n += 1) t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
	return t >>> 0;
...
function np(e, t) {
	return tp(`tsg-orbital-state-v1|${e}|${t}`) / 4294967296 * 360;
}
function rp(e) {
...
function ip(e) {
	let t = e.worlds.filter((e) => e.worldKind !== "Empty Orbit" && e.physical), n = new Map(t.map((e) => [e.id, e])), r = /* @__PURE__ */ new Map(), i = (t, a = /* @__PURE__ */ new Set()) => {
		...
		let s = n.get(t);
		if (!s || a.has(t)) return np(e.id, t);
		a.add(t);
		let c = s.trojanReferenceId && n.has(s.trojanReferenceId) ? i(s.trojanReferenceId, a) + (s.trojanOffsetDegrees ?? 0) : np(e.id, s.id);
		a.delete(t);
		let l = rp(c);
		return r.set(t, l), l;
	};
	return t.map((e) => ({
		...
		semiMajorAxisKm: e.au * $f,
		eccentricity: e.eccentricity,
		periodSeconds: e.physical.orbitalPeriodYears * ep,
...
function op(e) {
	...
	for (let n of e.worlds) (n.physical?.details?.satellites?.moons ?? []).forEach((r, i) => {
		...
		t.push({
			...
			meanAnomalyAtEpochDegrees: np(e.id, a),
			direction: r.direction
		});
	});
	return t;
...
var lp = "traveller-system-generator";
function up() {
	return {
		moduleId: lp,
		generateSystem(e = {}) {
			return e.method === "continuation" ? Bf(e) : Xf(e);
		},
		createTwodsixWorldActor(e) {
			let t = x(e), n = t.flags[lp];
			return t.flags[lp] = {
				...n && typeof n == "object" ? n : {},
...
function dp(e, t = up()) {
	let n = e.get(lp);
	if (!n) throw Error(`Foundry module ${lp} is not registered.`);
	return n.api = t, t;
}
//#endregion
...
function I(e, t) {
	return `<div style="display:grid;grid-template-columns:minmax(10rem,0.9fr) minmax(0,1.3fr);gap:0.75rem;padding:0.28rem 0;border-bottom:1px solid var(--color-border-light-primary);"><strong>${fp(e)}</strong><span style="min-width:0;overflow-wrap:anywhere;">${fp(t ?? "—")}</span></div>`;
}
function mp(e, t) {
	return `<details style="margin-top:0.55rem;padding:0.45rem;border:1px dashed var(--color-border-light-primary);border-radius:4px;"><summary style="cursor:pointer;font-weight:600;">${fp(e)}</summary><div style="margin-top:0.4rem;">${t}</div></details>`;
}
function hp(e) {
	return typeof e == "number" && Number.isFinite(e) ? `${e >= 0 ? "+" : ""}${e}` : "—";
}
function gp(e) {
	let t = pp(e).map((e) => String(e));
	return t.length ? `<div style="padding:0.35rem 0;"><strong>Notes</strong><ul style="margin:0.3rem 0 0 1.25rem;">${t.map((e) => `<li>${fp(e)}</li>`).join("")}</ul></div>` : "";
}
function _p(e, t) {
	if (!t) return I(e, "Unavailable");
	if (t.status !== "generated" || typeof t.lawLevel != "number") return [I(e, "Incomplete"), mp(`${e} Calculation`, [
		I("Status", t.status ?? "incomplete"),
		I("Overall Law Level", t.overallLawLevel ?? "—"),
		gp(t.generationNotes)
	].join(""))].join("");
	let n = pp(t.dmBreakdown).map(F).filter((e) => !!e).map((e) => `${String(e.source ?? "Modifier")}: DM${hp(e.dm)}`).join("; ");
	return [I(e, t.lawLevel), mp(`${e} Calculation`, [
		I("Overall Law Level", t.overallLawLevel ?? "—"),
		I("2D3 roll", t.roll ?? "—"),
		I("2D3 - 4 variance", hp(t.variance)),
		I("DM total", hp(t.dmTotal)),
		n ? I("Modifiers", n) : "",
...
function vp(e) {
	return e ? [
		...
		mp("Subclassifications", [
			_p("Weapons & Armour", F(e.weaponsAndArmour)),
			_p("Economic", F(e.economic)),
			_p("Criminal", F(e.criminal)),
			_p("Private", F(e.privateLaw)),
			_p("Personal Rights", F(e.personalRights)),
			gp(e.generationNotes)
		].join(""))
...
function bp(e, t) {
	for (let n of pp(F(e)?.balkanisedFactions)) {
		let e = F(n);
...
function R(e, t) {
	return `<div style="display:grid;grid-template-columns:minmax(10rem,0.9fr) minmax(0,1.3fr);gap:0.75rem;padding:0.28rem 0;border-bottom:1px solid var(--color-border-light-primary);"><strong>${Sp(e)}</strong><span style="min-width:0;overflow-wrap:anywhere;">${Sp(t ?? "—")}</span></div>`;
}
function wp(e, t) {
	return `<details style="margin-top:0.55rem;padding:0.45rem;border:1px dashed var(--color-border-light-primary);border-radius:4px;"><summary style="cursor:pointer;font-weight:600;">${Sp(e)}</summary><div style="margin-top:0.4rem;">${t}</div></details>`;
}
function Tp(e) {
	let t = Cp(e).map((e) => String(e));
	return t.length ? `<div style="padding:0.35rem 0;"><strong>Notes</strong><ul style="margin:0.3rem 0 0 1.25rem;">${t.map((e) => `<li>${Sp(e)}</li>`).join("")}</ul></div>` : "";
}
function Ep(e) {
	return typeof e == "number" && Number.isFinite(e) ? `${e >= 0 ? "+" : ""}${e}` : "—";
}
...
function Op(e) {
	...
	let t = L(e.secondarySystem), n = Cp(e.dmBreakdown).map(L).filter((e) => !!e).map((e) => `${String(e.source ?? "Modifier")}: DM${Ep(e.dm)}`).join("; "), r = [
		R("Status", e.status ?? "—"),
		R("2D roll", e.roll ?? "—"),
		R("DM total", Ep(e.dmTotal)),
		n ? R("Modifiers", n) : "",
		...
		Tp(e.generationNotes)
	].join(""), i = t ? [
		R("Scope", t.scope ?? "economic and regulatory"),
		R("2D roll", t.roll ?? "Not required"),
		R("Law Level DM", Ep(t.lawLevelDm)),
		R("Final total", t.total ?? "—"),
		R("Changed from primary", t.changedFromPrimary === !0 ? "Yes" : "No"),
		Tp(t.generationNotes)
	].join("") : "";
	return [
		R("Primary Judicial System", Dp(e)),
		t ? R("Secondary Judicial System", Dp(t)) : R("Secondary Judicial System", "—"),
		wp("Judicial System Calculation", r),
		t ? wp("Secondary System Calculation", i) : ""
	].join("");
}
function kp(e) {
	return e ? [R("Law Uniformity", `${String(e.code ?? "—")} — ${String(e.uniformity ?? "—")}`), wp("Law Uniformity Calculation", [
...
function Ap(e) {
	return e ? [R("Presumption of Innocence", e.code === "Y" ? "Y — Yes" : e.code === "N" ? "N — No" : "Unavailable"), wp("Presumption Calculation", [
		R("Status", e.status ?? "—"),
		R("2D roll", e.roll ?? "—"),
		R("Law Level DM", Ep(e.lawLevelDm)),
		R("Adversarial DM", Ep(e.adversarialDm)),
		R("DM total", Ep(e.dmTotal)),
		R("Final total", e.total ?? "—"),
		Tp(e.generationNotes)
...
function jp(e) {
	return e ? [R("Death Penalty", e.code === "Y" ? "Y — Yes" : e.code === "N" ? "N — No" : "Unavailable"), wp("Death Penalty Calculation", [
		R("2D roll", e.roll ?? "—"),
		R("Government DM", Ep(e.governmentDm)),
		R("Law Level DM", Ep(e.lawLevelDm)),
		R("DM total", Ep(e.dmTotal)),
		R("Final total", e.total ?? "—"),
		Tp(e.generationNotes)
...
function Pp(e, t) {
	for (let n of Cp(L(e)?.balkanisedFactions)) {
		let e = L(n);
...
function Ip(e) {
	...
	let t = wp("Law Level Profile & Subclassifications", xp(e)), n = L(e.justiceProfileDetails), r = L(e.judicialSystemDetails), i = L(e.lawUniformityDetails), a = L(e.presumptionOfInnocenceDetails), o = L(e.deathPenaltyDetails);
	if (!(n || r || i || a || o)) return t;
	if (e.governmentCode !== 7) return [t, wp("Justice Profile & Systems", Fp(Np(n), Np(r), Np(i), Np(a), Np(o)))].join("");
	...
	for (let e of [
		...
		a,
		o
	]) for (let t of Cp(e?.balkanisedFactions)) {
		let e = L(t);
		typeof e?.factionId == "string" && s.add(e.factionId);
	}
	return s.size ? [t, wp("Justice Profile & Systems", ["<p class=\"hint\">Government 7 has no single world-level Justice Profile. Each represented sovereign faction is shown separately.</p>", ...Array.from(s).map((e) => `<div style="margin-top:0.65rem;padding:0.55rem;border:1px solid var(--color-border-light-primary);border-radius:5px;"><h4 style="margin:0 0 0.35rem;">${Sp(e)}</h4>${Fp(Pp(n, e), Pp(r, e), Pp(i, e), Pp(a, e), Pp(o, e))}</div>`)].join(""))].join("") : [t, "<p class=\"hint\">Government 7 uses faction-specific Justice results, but no represented sovereign-faction results are stored.</p>"].join("");
}
//#endregion
...
var Lp = {
	...
	format(e, t) {
		return Object.entries(t).reduce((e, [t, n]) => e.replaceAll(`{${t}}`, String(n)), Lp[e]);
	}
};
//#endregion
...
function Wp(e) {
	...
	for (let e of n.values()) e.sort(Vp);
	...
	for (let t of [...e].sort(Vp)) i.has(t.id) || a(t, 0, []);
	return r;
}
function Gp(e, t, n = "") {
	let r = Wp(e).map((e) => {
		let t = e.depth > 0 ? `${"\xA0\xA0".repeat(e.depth)}└─ ` : "", r = e.id === n ? " selected" : "";
		return `<option value="${Bp(e.id)}"${r}>${Bp(`${t}${e.path}`)}</option>`;
	});
	return [`<option value=""${n ? "" : " selected"}>${Bp(t.localize("TSG.Dialog.ActorFolderRoot"))}</option>`, ...r].join("");
}
function Kp(e = 1105, t = [], n = Rp, r = "") {
	let i = n.localize;
	return `
    <div class="form-group"><label>${i("TSG.Dialog.SystemName")}</label><input name="name" type="text" value="${Bp(i("TSG.Dialog.DefaultSystemName"))}" autofocus></div>
    <div class="form-group"><label>${i("TSG.Dialog.Seed")}</label><input name="seed" type="number" value="${Bp(String(e))}" step="1"></div>
    ...
    <div class="form-group"><label>${i("TSG.Dialog.ActorFolder")}</label><select name="folder">${Gp(t, n, r)}</select></div>
...
function Xp(e) {
	let t = qp(e.name, "Uncharted System").trim() || "Uncharted System", n = Jp(e.seed, 1105), r = qp(e.method, "expanded"), i = {
		name: t,
		seed: n,
		settings: {
			starDistribution: qp(e.starDistribution, "classic") === "realistic" ? "realistic" : "classic",
			detailLevel: ["basic", "deep"].includes(qp(e.detailLevel)) ? qp(e.detailLevel) : "standard",
			allowUnusualPrimaries: Yp(e.allowUnusualPrimaries, !0),
			populationMode: qp(e.populationMode, "established") === "survey" ? "survey" : "established"
		}
	};
	return {
		request: r === "continuation" ? {
			...i,
			method: "continuation",
			sourceUwp: qp(e.sourceUwp, "A867A74-C").trim().toUpperCase()
		} : {
			...i,
			method: "expanded"
		},
		folder: qp(e.folder).trim() || null,
		openSheet: Yp(e.openSheet, !0)
	};
}
function Zp(e) {
	return Xp(e).request;
...
async function Qp(e, t) {
	...
	let r = e.listActorFolders(), i = e.defaultSystemFolderId?.() ?? "", a = await e.prompt({
		...
		rejectClose: !1,
		modal: !1,
		render: (e, t) => Hp(t.element)
	});
	if (a) try {
		let r = Xp(a), i = t.generateSystem(r.request), o = t.createTwodsixWorldActor(i), s = o.flags && typeof o.flags == "object" ? o.flags : {}, c = s["traveller-system-generator"] && typeof s["traveller-system-generator"] == "object" ? s["traveller-system-generator"] : {}, l = Number(r.request.seed ?? 1105);
		s["traveller-system-generator"] = {
			...c,
			generationSeed: l,
			generationSeedProvenance: zp(l, 0)
...
var $p = "traveller-system-generator", em = [
	"notes",
	"adventureHooks",
	"relatedActors",
	"worldImage"
...
function rm(e) {
	return !!(e && e.type === "world" && nm(e));
}
function im(e) {
	return !!(e && e.type === "world" && !nm(e));
}
function am(e, t, { adopting: n = !1 } = {}) {
	let r = nm(e) ?? {}, i = r.generationSettings ?? {}, a = String(e.system.name ?? e.name.replace(/ System$/, "")), o = r.generationMethod ?? "expanded", s = r.sourceUwp ?? String(e.system.uwp ?? "A867A74-C"), c = r.generationSeedProvenance?.inputSeed ?? r.generationSeed ?? (n ? Up() : 1105), l = i.starDistribution ?? "classic", u = i.detailLevel ?? "standard", d = i.allowUnusualPrimaries ?? !0, f = i.populationMode ?? "established", p = t.localize, m = p(n ? "TSG.Dialog.AdoptHint" : "TSG.Dialog.RegenerateHint"), h = n ? `<select name="method"><option value="expanded"${o === "expanded" ? " selected" : ""}>${p("TSG.Dialog.MethodExpanded")}</option><option value="continuation"${o === "continuation" ? " selected" : ""}>${p("TSG.Dialog.MethodContinuation")}</option></select>` : `<input name="method" type="hidden" value="${o}"><span>${p(o === "expanded" ? "TSG.Dialog.MethodExpanded" : "TSG.Dialog.MethodContinuation")}</span>`, g = o === "continuation" || n ? `<div class="form-group"><label>${p("TSG.Dialog.SourceUwp")}</label><input name="sourceUwp" type="text" value="${tm(s)}" maxlength="9"><p class="hint">${p("TSG.Dialog.SourceUwpHint")}</p></div>` : `<input name="sourceUwp" type="hidden" value="${tm(s)}">`, _ = o === "expanded" ? `<div class="form-group"><label>Expanded System Population</label><select name="populationMode"><option value="established"${f === "established" ? " selected" : ""}>Established / Settled</option><option value="survey"${f === "survey" ? " selected" : ""}>Survey / Unexplored</option></select></div>` : `<input name="populationMode" type="hidden" value="${f}">`;
	return `
    <p class="hint">${tm(m)}</p>
    <div class="form-group"><label>${p("TSG.Dialog.SystemName")}</label><input name="name" type="text" value="${tm(a)}" autofocus></div>
    <div class="form-group"><label>${p("TSG.Dialog.Seed")}</label><input name="seed" type="number" value="${c}" step="1"></div>
    <div class="form-group"><label>${p("TSG.Dialog.Method")}</label>${h}</div>
    ${_}
    ${g}
    <div class="form-group"><label>${p("TSG.Dialog.StarDistribution")}</label><select name="starDistribution"><option value="classic"${l === "classic" ? " selected" : ""}>${p("TSG.Dialog.DistributionClassic")}</option><option value="realistic"${l === "realistic" ? " selected" : ""}>${p("TSG.Dialog.DistributionRealistic")}</option></select></div>
    <div class="form-group"><label>${p("TSG.Dialog.DetailLevel")}</label><select name="detailLevel"><option value="basic"${u === "basic" ? " selected" : ""}>${p("TSG.Dialog.DetailBasic")}</option><option value="standard"${u === "standard" ? " selected" : ""}>${p("TSG.Dialog.DetailStandard")}</option><option value="deep"${u === "deep" ? " selected" : ""}>${p("TSG.Dialog.DetailDeep")}</option></select></div>
    <div class="form-group"><label class="checkbox"><input name="allowUnusualPrimaries" type="checkbox"${d ? " checked" : ""}> ${p("TSG.Dialog.AllowUnusualPrimaries")}</label></div>
    <div class="form-group"><label class="checkbox"><input name="openSheet" type="checkbox" checked> ${p("TSG.Dialog.OpenSheet")}</label></div>`;
}
function om(e, t) {
	return am(e, t);
}
function sm(e, t) {
	return am(e, t, { adopting: !0 });
}
function cm(e, t, n) {
	let r = { ...t.system };
	for (let t of em) e.system[t] !== void 0 && (r[t] = e.system[t]);
	let i = { ...t.flags }, a = nm(e);
	return i[$p] = {
		...i[$p] ?? {},
		...
		generationSeed: n,
		generationSeedProvenance: zp(n, 0)
	}, {
		name: e.name,
		img: e.img,
		folder: e.folder,
...
async function lm(e, t, n, r) {
	...
	await e.update(cm(e, s, Number(a.seed ?? 1105))), t.openSheet !== !1 && e.sheet?.render(!0);
}
async function um(e, t) {
	...
	if (r) try {
		await lm(n, r, e, t), e.notifyInfo(e.localizer.format("TSG.Notification.Regenerated", { name: n.name }));
	} catch (t) {
...
async function dm(e, t) {
	...
	if (r) try {
		await lm(n, r, e, t), e.notifyInfo(e.localizer.format("TSG.Notification.Adopted", { name: n.name }));
	} catch (t) {
		let n = t instanceof Error ? t.message : String(t);
		e.notifyError(e.localizer.format("TSG.Notification.Failed", { message: n }));
	}
...
var fm = "traveller-system-generator";
function pm(e) {
	let t = e.flags?.[fm];
	return t && typeof t == "object" ? t : null;
}
...
async function hm(e, t, n) {
	...
	return await e.update(cm(e, a, Number(r.seed))), e.sheet?.render(!0), !0;
}
//#endregion
...
function Tm(e) {
	...
	let t = _m(e.functions).map((e) => B(e)).filter((e) => !!e);
...
function Em(e) {
	...
	return [
		...
		Array.isArray(e.dmBreakdown) && e.dmBreakdown.length ? `<div style="padding:0.35rem 0;"><strong>Authority modifiers</strong><ul style="margin:0.3rem 0 0 1.25rem;">${e.dmBreakdown.map((e) => `<li>${z(Sm(e))}</li>`).join("")}</ul></div>` : "",
		Tm(B(e.functionalStructure)),
...
function Dm(e) {
	...
	let t = _m(e.factions).map((e) => B(e)).filter((e) => !!e), n = _m(e.relationships).map((e) => B(e)).filter((e) => !!e);
	return [
		...
		n.length ? `<div style="padding:0.35rem 0;"><strong>Faction relationships</strong>${n.map((e) => {
			let t = _m(e.dmBreakdown).map(Sm).join("; ");
...
function Om(e) {
	let t = _m(e.cities).map((e) => B(e)).filter((e) => !!e), n = [
		V("Population allocation case", e.populationAllocationCase ?? "—"),
		e.largestNonMajorCityPopulation !== null && e.largestNonMajorCityPopulation !== void 0 ? V("Largest non-major city", bm(e.largestNonMajorCityPopulation)) : "",
		...
		e.allocationChunkPercent !== null && e.allocationChunkPercent !== void 0 ? V("Allocation chunk size", wm(e.allocationChunkPercent, 2)) : "",
		e.allocationRemainderPercent !== null && e.allocationRemainderPercent !== void 0 ? V("Allocation remainder", wm(e.allocationRemainderPercent, 2)) : ""
	].join("");
	return t.length ? `${n}<div style="padding:0.45rem 0 0.15rem;"><strong>Individual city allocations</strong></div>${t.map((e, t) => {
		let n = _m(e.allocationRolls).map((e) => String(e)).join(", "), r = e.chunkCount === null || e.chunkCount === void 0 ? "—" : String(e.chunkCount);
		return V(`City ${e.rank ?? t + 1}`, `${bm(e.population)} — ${wm(e.sharePercent, 2)} share — chunks ${r} — rolls ${n || "—"}`);
	}).join("")}` : n;
}
function km(e) {
	return e ? [
		...
		Array.isArray(e.dmBreakdown) && e.dmBreakdown.length ? `<div style="padding:0.35rem 0;"><strong>Centralisation modifiers</strong><ul style="margin:0.3rem 0 0 1.25rem;">${e.dmBreakdown.map((e) => `<li>${z(Sm(e))}</li>`).join("")}</ul></div>` : "",
		Em(B(e.authority))
...
function Am(e) {
	...
	let t = _m(e.factions).map((e) => B(e)).filter((e) => !!e);
	return [
		...
		t.length ? `<div style="padding:0.35rem 0;"><strong>Faction governments</strong>${t.map((e) => {
			let t = B(e.centralisation), n = B(t?.authority), r = B(n?.functionalStructure), i = B(n?.governmentProfile), a = _m(r?.functions).map((e) => B(e)).filter((e) => !!e).map((e) => `${String(e.functionCode ?? "—")}${String(e.code ?? "—")}`).join("/"), o = t ? ` — centralisation ${String(t.code ?? "—")} (${String(t.roll ?? "—")} ${typeof t.dmTotal == "number" ? `${t.dmTotal >= 0 ? "+" : ""}${t.dmTotal}` : ""} = ${String(t.total ?? "—")})` : "", s = n ? ` — authority ${String(n.code ?? "—")} ${String(n.authoritativeFunction ?? "—")} (${String(n.roll ?? "—")} ${typeof n.dmTotal == "number" ? `${n.dmTotal >= 0 ? "+" : ""}${n.dmTotal}` : ""} = ${String(n.total ?? "—")})` : "", c = a ? ` — structure ${a}` : "", l = i ? ` — profile ${String(i.profile ?? "—")}` : "";
			return [V(String(e.id ?? "Faction"), `${vm(e.governmentCode)} — ${String(e.governmentType ?? "—")} — 2D ${String(e.governmentRoll ?? "—")} ${typeof e.governmentModifier == "number" ? `${e.governmentModifier >= 0 ? "+" : ""}${e.governmentModifier}` : ""} = ${String(e.governmentUnclampedTotal ?? "—")}${o}${s}${c}${l}`), `<div style="margin-left:0.8rem;padding:0.25rem 0 0.45rem;"><strong>${z(String(e.id ?? "Faction"))} internal factions</strong>${Dm(B(e.internalFactions))}</div>`].join("");
		}).join("")}</div>` : "",
...
function jm(e) {
	...
	let t = B(e.populationDetails), n = B(e.populationConcentration), r = B(e.urbanisation), i = B(e.majorCities), a = B(e.governmentDetails), o = B(e.governmentCentralisation), s = B(e.governmentFactions), c = B(e.balkanisation), l = _m(e.tradeCodes).map((e) => String(e)).join(" "), u = _m(e.minimumSustainableTechLevelBasis).map((e) => String(e)), d = typeof e.notes == "string" ? e.notes : null, f = gm(t, n, r, i), p = [V("Habitation", xm(e.habitationStatus)), V("Basis", e.habitationBasis ?? "—")].join(""), m = t ? [
		V("WBH method", t.method ?? "—"),
		V("Population phase-1 prefix", t.profilePrefix ?? "—"),
		V("Population code", vm(t.populationCode)),
		V("P value", t.pValue ?? "—"),
		V("Additional significant digit", t.additionalSignificantDigit ?? "—"),
		V("Estimated population", bm(t.estimatedPopulation)),
		...
		Array.isArray(n.dmBreakdown) && n.dmBreakdown.length ? `<div style="padding:0.35rem 0;"><strong>PCR modifiers</strong><ul style="margin:0.3rem 0 0 1.25rem;">${n.dmBreakdown.map((e) => `<li>${z(Sm(e))}</li>`).join("")}</ul></div>` : "",
		...
		V("Total urban population", bm(r.totalUrbanPopulation)),
		V("PCR used", r.pcr ?? "—"),
		...
		B(r.appliedMinimum) ? V("Applied minimum", Cm(r.appliedMinimum)) : "",
		B(r.appliedMaximum) ? V("Applied maximum", Cm(r.appliedMaximum)) : "",
		Array.isArray(r.dmBreakdown) && r.dmBreakdown.length ? `<div style="padding:0.35rem 0;"><strong>Urbanisation modifiers</strong><ul style="margin:0.3rem 0 0 1.25rem;">${r.dmBreakdown.map((e) => `<li>${z(Sm(e))}</li>`).join("")}</ul></div>` : "",
		Array.isArray(r.minimumLimits) && r.minimumLimits.length ? `<div style="padding:0.35rem 0;"><strong>Minimum restrictions</strong><ul style="margin:0.3rem 0 0 1.25rem;">${r.minimumLimits.map((e) => `<li>${z(Cm(e))}</li>`).join("")}</ul></div>` : "",
		Array.isArray(r.maximumLimits) && r.maximumLimits.length ? `<div style="padding:0.35rem 0;"><strong>Maximum restrictions</strong><ul style="margin:0.3rem 0 0 1.25rem;">${r.maximumLimits.map((e) => `<li>${z(Cm(e))}</li>`).join("")}</ul></div>` : "",
		...
		V("Combined major-city population", bm(i.totalMajorCityPopulation)),
		V("Share of urban population", wm(i.totalMajorCitySharePercent)),
		...
		V("Profile", f.profile),
		V("Format", "P-p.pp-C-%%-M"),
		V("Population", vm(f.populationCode)),
		...
		V("Government code", vm(a.governmentCode)),
		...
		V("Government code", vm(e.governmentCode)),
		V("Law Level", vm(e.lawLevelCode)),
		V("Tech Level", vm(e.techLevel)),
	...
	return [
		ym("Habitation", p),
		ym("Population", m),
		ym("Population Concentration", h),
		ym("Urbanisation", g),
		ym("Major Cities", _),
		ym("Population Profile", v),
		ym("Government", y),
		ym("Centralisation, Authority, Structure & Profile", km(o)),
		ym("Government Factions", e.governmentCode === 7 ? "<p class=\"hint\">Government 7 uses the sovereign Balkanisation layer below; internal factions are shown inside each sovereign government.</p>" : Dm(s)),
		ym("Balkanisation", Am(c)),
		ym("Current UWP Social Values", b)
	].join("");
}
//#endregion
//#region src/integrations/foundry/managerControl.ts
var Mm = "traveller-system-generator", Nm = "__all__", Pm = "__root__";
function H(e) {
	return e.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\"/g, "&quot;").replace(/'/g, "&#039;");
}
function Fm(e) {
...
function Lm(e, t = "", n = Nm) {
	return Im(e).filter((e) => n === Nm || (n === Pm ? !e.folderId : e.folderId === n)).map((e) => {
		let n = rm(e), r = `${e.name}${n ? "" : " — not generator-managed"}`;
		return `<option value="${H(e.id)}" data-folder-id="${H(e.folderId ?? "")}" data-generator-managed="${n ? "true" : "false"}"${e.id === t ? " selected" : ""}>${H(r)}</option>`;
	}).join("");
}
function Rm(e, t = Nm) {
	let n = e.localizer.localize("TSG.Dialog.ActorFolderRoot"), r = Wp(e.listActorFolders());
	return [
		`<option value="${Nm}"${t === Nm ? " selected" : ""}>All Actor folders</option>`,
		`<option value="${Pm}"${t === Pm ? " selected" : ""}>${H(n)}</option>`,
		...r.map((e) => `<option value="${H(e.id)}"${e.id === t ? " selected" : ""}>${H(e.path)}</option>`)
	].join("");
}
function zm(e) {
	if (!e) return null;
	let t = e.flags?.[Mm], n = t && typeof t == "object" ? t : {}, r = n.generationSettings, i = r && typeof r == "object" ? r : {}, a = rm(e);
	return {
...
function Vm(e, t) {
	let n = t.localizer.localize, r = zm(e);
    ...
    <div style="padding:0.75rem;border:1px solid var(--color-border-light-primary);border-radius:6px;background:var(--color-bg-option);min-width:0;">
      ...
      ${Bm("Generator status", r.managed ? "Generator-managed" : "Not generator-managed")}
      ${Bm(n("TSG.Manager.Folder"), r.folderPath)}
      ${Bm(n("TSG.Manager.Uwp"), r.uwp)}
      ${Bm(n("TSG.Manager.Method"), r.method)}
      ${Bm(n("TSG.Manager.Seed"), r.seed)}
      ${Bm(n("TSG.Manager.Distribution"), r.distribution)}
      ${Bm(n("TSG.Manager.DetailLevel"), r.detailLevel)}
      ${r.managed ? "" : `<p class="hint" style="margin:0.6rem 0 0;">${H(n("TSG.Manager.AdoptHint"))}</p>`}
    </div>`;
}
function Hm(e, t, n, r, i = !1) {
	return `
...
function Um(e) {
	return e?.folderId || (e ? Pm : Nm);
}
function Wm(e, t, n = {}) {
	let r = t.localizer.localize, i = Im(e), a = i.find((e) => e.id === n.initialActorId) ?? i[0], o = n.initialActorId ? Um(a) : Nm, s = o === Nm ? i : i.filter((e) => o === Pm ? !e.folderId : e.folderId === o), c = s.find((e) => e.id === a?.id) ?? s[0], l = Lm(i, c?.id, o), u = n.initialAction ?? "generate", d = i.length > 0, f = !!(c && rm(c)), p = !!(c && im(c));
	return `
    <section style="display:grid;gap:0.8rem;min-width:0;">
      ...
      ${Hm("generate", r("TSG.Manager.Generate"), r("TSG.Manager.GenerateDescription"), u === "generate")}
    </section>
    ...
    <section style="display:grid;gap:0.8rem;min-width:0;">
      ...
      <div class="form-group" style="min-width:0;"><label>Actor Folder</label><select name="managerFolderId" aria-label="Actor Folder" style="width:100%;min-width:0;max-width:100%;">${Rm(t, o)}</select></div>
      ...
      <div data-manager-actor-details style="min-width:0;">${Vm(c, t)}</div>
      <div style="display:grid;grid-template-columns:1fr;gap:0.75rem;">
        ${Hm("open", r("TSG.Manager.Open"), r("TSG.Manager.OpenDescription"), u === "open", !d)}
        ${Hm("update", r("TSG.Manager.Update"), r("TSG.Manager.UpdateDescription"), u === "update" && f, !f)}
        ${Hm("adopt", r("TSG.Manager.Adopt"), r("TSG.Manager.AdoptDescription"), u === "adopt" && p, !p)}
      </div>
    </section>`;
}
function Gm(e) {
...
function Km(e, t, n) {
	...
	i.addEventListener("change", () => {
		let e = i.value || Nm, t = a.value, n = Lm(r, t, e);
...
async function Jm(e, t, n = {}) {
	...
	let r = e.listGeneratedActors(), i = await e.prompt({
		...
		rejectClose: !1,
		modal: !1,
		render: (t, n) => Km(n.element, r, e)
	});
	if (!i) return;
	let a = Gm(i.action) || n.initialAction || "generate";
	if (a === "generate") {
		await Qp(e, t);
		return;
	}
	let o = Im(r).find((e) => e.id === Gm(i.actorId));
	if (!o) {
...
var Xm = "traveller-system-generator";
function U(e) {
	return e && typeof e == "object" && !Array.isArray(e) ? e : null;
}
function W(e) {
...
function Qm(e) {
	return U(U(e.flags?.[Xm])?.sensorSystem);
}
function $m(e) {
	let t = Qm(e);
...
function th(e) {
	return (Qm(e)?.bodies ?? []).flatMap((e, t) => {
		let n = U(e);
		if (!n) return [];
...
function oh(e, t, n, r) {
	...
	if (e !== null) {
		let t = Zm(a?.diameterKm);
		c.push(`Size ${nh(e)} → ${t === null ? "no precise diameter" : `${q(t, 0, " km")} precise diameter`} ${t === null ? "⚠" : "✓"}`);
	}
	if (t !== null) {
		let e = G(o?.classification), n = Zm(o?.meanBaselinePressureBar);
		c.push(`Atmosphere ${nh(t)} → ${e ?? "no detailed classification"}${n === null ? "" : `, ${q(n, 3, " bar")}`} ${e ? "✓" : "⚠"}`);
	}
	if (n !== null) {
		let e = Zm(s?.coveragePercent), t = n === 0 ? 0 : Math.max(0, n * 10 - 5), r = n === 10 ? 100 : Math.min(100, n * 10 + 5), i = e !== null && e >= t && e <= r;
		c.push(`Hydrographics ${nh(n)} → ${e === null ? "no precise coverage" : `${q(e, 1, "%")} coverage`} ${e === null ? "⚠" : i ? "✓" : "⚠ review"}`);
	}
...
function uh(e) {
	...
	return [
		J("Hydrographics code", nh(e?.hydrographicsCode)),
		...
		J("Runaway greenhouse eligible", rh(r?.runawayGreenhouseEligible))
	].join("");
}
function dh(e) {
...
function ph(e) {
	...
	return [
		...
		J("Water-based tectonics eligible", rh(t.waterBasedTectonicsEligible)),
		ih(n)
	].join("");
}
function mh(e) {
	...
	return [
		...
		ih([J("Generation policy", t.generationPolicy ?? "—"), ah(t.generationNotes)].join(""))
	].join("");
}
function hh(e) {
	...
	return [
		...
		J("Agriculture thermally eligible", rh(t.agricultureThermallyEligible)),
		J("Unprotected settlement thermally eligible", rh(t.unprotectedSettlementThermallyEligible)),
...
function _h(e) {
	...
	return [
		...
		J("Current native sophont", rh(t.currentNativeSophont)),
		J("Extinct native sophont evidence", rh(t.extinctNativeSophontEvidence)),
		ih(r)
	].join("");
}
function vh(e) {
	...
	return [J("Resource Rating", `${t.code ?? "—"} (${t.rating ?? "—"})`), ih(r)].join("");
}
function yh(e) {
	...
	let n = U(t.dm), r = [
		...
		J("Total DM", n?.total ?? "—"),
		J("Unclamped total", t.unclampedTotal ?? "—"),
		J("Detailed temperatures used", rh(t.usedDetailedTemperature)),
		J("Computed gravity used", rh(t.usedComputedGravity)),
		ah(t.generationNotes)
	].join("");
	return [
...
function xh(e) {
	let t = U(Qm(e)?.mainworldDetermination);
...
function Ch(e) {
	let t = e.physical, n = Zm(t?.sizeCode), r = Zm(t?.atmosphereCode), i = Zm(t?.hydrographicsCode), a = G(e.social?.uwp), o = JSON.stringify(e.source, null, 2);
	return `
    <div data-wbh-body-panel="${K(e.id)}" style="display:grid;gap:0.7rem;min-width:0;">
      ...
      ${Y("UWP / Detailed Consistency", oh(n, r, i, t))}
      ${Y("Social Characteristics", jm(e.social), !1)}
      ${Y("Justice", Ip(e.social), !1)}
      ${Y("Physical", sh(t))}
      ${Y("Atmosphere", ch(t), !1)}
      ${Y("Hydrographics / Climate", uh(t), !1)}
      ${Y("Temperature Extremes", dh(t), !1)}
      ${Y("Rotation / Tides", fh(t), !1)}
      ${Y("Seismology", ph(t), !1)}
      ${Y("Surface Geology", mh(t), !1)}
      ${Y("Surface Climate", hh(t), !1)}
      ${Y("Satellites", gh(t), !1)}
      ${Y("Native Life", _h(t), !1)}
      ${Y("Resource Rating", vh(t), !1)}
      ${Y("Habitability Rating", yh(t), !1)}
...
function wh(e, t) {
    ...
    ${Y("Final Mainworld Determination", xh(e), !0)}
    <div data-wbh-selected-body>${Ch(t)}</div>
  </div>`;
}
function Th(e) {
	let t = th(e), n = t.find((e) => e.isMainworld) ?? t[0];
	return n ? `
    <div style="display:grid;gap:0.75rem;min-width:0;">
      ...
      <div data-wbh-inspector-panel>${wh(e, n)}</div>
...
async function Dh(e, t, n) {
	...
	let r = G(U(Qm(e)?.mainworldDetermination)?.selectedMainworldId), i = await t.prompt({
		window: { title: `WBH Detail Inspector — ${e.name}` },
		content: Th(e),
		ok: { label: "Close" },
		rejectClose: !1,
		modal: !1,
		render: (t, n) => Eh(n.element, e)
	});
	if (!i || !n) return;
	let a = G(i.wbhMainworldOverrideId);
	if (!(!a || a === r)) try {
		await hm(e, a, n) ? t.notifyInfo(`Mainworld changed to ${a}. The WBH generated recommendation remains recorded.`) : t.notifyWarning("Mainworld override is only available for generator-managed expanded systems.");
	} catch (e) {
...
var Oh = "traveller-system-generator";
function kh(e) {
	return e && typeof e == "object" && !Array.isArray(e) ? e : null;
}
function Ah(e) {
...
function Mh(e) {
	let t = kh(kh(e.sensorSystem)?.mainworldDetermination);
	if (t?.selectionSource === "referee-override") return Ah(t.selectedMainworldId) ?? void 0;
}
function Nh(e) {
	return String(e ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\"/g, "&quot;").replace(/'/g, "&#039;");
}
...
function Fh(e, t) {
	let n = jh(e);
	if (!n || n.generationMethod === "continuation" || !Number.isFinite(n.generationSeed)) return null;
	let r = n.generationSettings ?? {}, i = Mh(n);
	return {
		method: "expanded",
		name: String(e.system.name ?? e.name.replace(/ System$/, "")),
		seed: n.generationSeed,
...
function Lh(e) {
	jh(e)?.habitationOverrides;
	let t = th(e).filter((e) => e.physical ? !kh(kh(e.physical.details)?.gasGiant) && e.kind !== "Gas Giant" : !1);
	return t.length ? `
    ...
    <div class="form-group">
      ...
      <select name="bodyId">
        ${t.map((e) => {
		let t = e.social, n = Ah(t?.habitationStatus) ?? "uninhabited";
		return `<option value="${Nh(e.id)}">${Nh(`${e.label} — ${n}`)}</option>`;
	}).join("")}
      </select>
    </div>
    <div class="form-group">
...
async function zh(e, t, n) {
	...
	let r = await t.prompt({
		...
		rejectClose: !1,
		modal: !1,
		render: (e, t) => Rh(t.element)
	});
	if (!r) return;
	let i = Ah(r.bodyId);
	if (!i) return;
	let a = th(e).find((e) => e.id === i);
	if (!a) {
		t.notifyError(`Unable to find body ${i}.`);
		return;
	}
	if (kh(kh(a.physical?.details)?.nativeLife)?.currentNativeSophont === !0) {
	...
	let o = { ...jh(e)?.habitationOverrides ?? {} }, s = Ah(r.habitationAction);
	if (s === "clear") delete o[i];
	else {
		let e = Ah(r.populationMode) ?? "generate", n = null;
		if (e === "manual") {
...
function Kh(e) {
	...
	let n = Vh(t.candidates).map(Bh).filter((e) => !!e), r = n.length ? `<div style="overflow:auto;margin-top:0.65rem;"><table style="width:100%;border-collapse:collapse;font-size:0.9em;">
        ...
        <tbody>${n.map((e) => {
		let t = Vh(e.minimumSustainableTechLevelBasis).map(String).join("; ");
		return `<tr>
            <td style="white-space:nowrap;">${X(e.parentId ? `${String(e.id ?? "—")} (moon of ${String(e.parentId)})` : String(e.id ?? "—"))}</td>
            <td>${X(e.kind ?? "—")}</td>
            <td>${X(e.existingPopulationCode === null || e.existingPopulationCode === void 0 ? "None" : Hh(e.existingPopulationCode))}</td>
            <td>${X(Uh(e.currentNativeSophonts))}</td>
            <td title="${X(t)}">${X(`TL${String(e.minimumSustainableTechLevel ?? "—")}`)}</td>
            <td>${X(Uh(e.sustainableAtMainworldTechLevel))}</td>
            <td>${X(Hh(e.effectiveMaximumPopulationCode))}</td>
            <td>${X(Gh(e.status))}</td>
          </tr>`;
	}).join("")}</tbody>
      </table></div>` : "<p class=\"hint\">No eligible physical secondary-world candidates were found after excluding the mainworld, gas giants, and empty orbits.</p>", i = Vh(t.generationNotes).map((e) => `<li>${X(e)}</li>`).join("");
	return [
		Wh("Mainworld", t.mainworldId ?? "—"),
		Wh("Mainworld Population", Hh(t.mainworldPopulationCode)),
		Wh("Mainworld Tech Level", `TL${Hh(t.mainworldTechLevel)}`),
		Wh("Normal individual maximum", `${Hh(t.normalMaximumPopulationCode)} — mainworld Population − 1`),
		Wh("Optional offworld 1D roll", t.offworldMaximumRoll ?? "—"),
		Wh("Optional system maximum", `${Hh(t.systemMaximumPopulationCode)} — mainworld Population − 1D`),
		Wh("Effective maximum", Hh(t.effectiveMaximumPopulationCode)),
		Wh("Secondary populations possible", Uh(t.secondaryPopulationsPossible)),
		r,
...
function Jh(e, t, n, r) {
	return `${e}${t.length ? ` (${t.join(", ")})` : ""}: ${qh(n)}: ${r ?? "—"}`;
}
function Yh(e) {
	return !e || e.numberMajorCities <= 0 || e.cities.length === 0 ? null : {
		...
		cities: [...e.cities].sort((e, t) => e.rank - t.rank).map((e) => {
			...
			return {
				...
				codes: n,
				port: null,
				profile: Jh(t, n, e.population, null)
			};
		}),
		generationNotes: [
...
function Zh(e) {
	return Xh.find((t) => t.code === e) ?? null;
}
function Qh(e, t) {
...
function $h(e) {
	...
	e.starport === "A" && e.hasHighport && n("Starport A with highport", 1), (e.starport === "E" || e.starport === "X") && n(`Starport ${e.starport}`, -2), [
		...
		1,
		10
	].includes(e.atmosphereCode) ? n("Atmosphere 0, 1, or A", 2) : e.atmosphereCode === 11 ? n("Atmosphere B", 3) : e.atmosphereCode === 12 && n("Atmosphere C", 4), e.techLevel >= 16 ? n("TL16+", 3) : e.techLevel >= 13 ? n("TL13–15", 2) : e.techLevel >= 9 && n("TL9–12", 1), Qh(e.tradeCodes, "In") && n("Industrial", 1), Qh(e.tradeCodes, "Ni") && n("Non-Industrial", -1), Qh(e.tradeCodes, "Ri") && n("Rich", 1), Qh(e.tradeCodes, "Po") && n("Poor", -1);
...
var ng = "traveller-system-generator";
function rg(e) {
	return e && typeof e == "object" && !Array.isArray(e) ? e : null;
}
function ig(e) {
...
function sg(e) {
	return rg(e.flags?.[ng]);
}
function cg(e) {
	let t = rg(sg(e)?.cityOverrides);
	if (!t) return {};
	let n = {};
	for (let [e, r] of Object.entries(t)) {
		let t = rg(r), i = ag(t?.cityRank), a = ig(t?.code), o = ig(t?.description), s = t?.source === "custom" ? "custom" : t?.source === "standard" ? "standard" : null;
		i !== null && a && o && s && (n[e] = {
			cityRank: i,
			code: a,
			description: o,
...
function lg(e) {
	return e?.source === "custom" ? {
		action: "custom",
		standardCode: Xh[0]?.code ?? "",
		customCode: e.code,
		customDescription: e.description
	} : {
		action: "standard",
		standardCode: e?.source === "standard" ? e.code : Xh[0]?.code ?? "",
		customCode: "",
		customDescription: ""
	};
}
function ug(e) {
	return th(e).find((e) => e.isMainworld);
}
function dg(e) {
	let t = ug(e)?.social, n = rg(t?.majorCities);
	return Array.isArray(n?.cities) ? n.cities.flatMap((e) => {
		let t = rg(e), n = ag(t?.rank), r = ag(t?.population);
...
function fg(e) {
	return e.type === "world" && !!ug(e)?.social && dg(e).length > 0;
}
function pg(e) {
	let t = ug(e), n = t?.social, r = t?.physical, i = dg(e), a = ag(n?.techLevel) ?? 0, o = ag(r?.atmosphereCode) ?? 0, s = Array.isArray(n?.tradeCodes) ? n.tradeCodes.map(String) : [], c = $h({
		starport: ig(n?.starport),
		hasHighport: !1,
		atmosphereCode: o,
		techLevel: a,
		tradeCodes: s
	}), l = cg(e), u = c.dmTotal >= 0 ? `+${c.dmTotal}` : String(c.dmTotal);
	return `
    ...
    <div style="padding:0.5rem;border:1px solid var(--color-border-light-primary);border-radius:6px;margin-bottom:0.65rem;">
      ...
      TL ${og(a)}; optional unusual-city occurrence DM ${og(u)}; natural 2D roll required ${og(c.effectiveNaturalRollRequired)}+.
      ${c.dmBreakdown.length ? `<div class="hint">${c.dmBreakdown.map((e) => `${og(e.source)}: DM${e.dm >= 0 ? "+" : ""}${e.dm}`).join("; ")}</div>` : ""}
    ...
    <div class="form-group"><label>Major city</label><select name="cityRank">
      ${i.map((e) => {
		...
		return `<option value="${e.rank}">Major City ${e.rank} — ${e.population.toLocaleString("en-US")}${t ? ` — currently ${og(t.code)}` : ""}</option>`;
	}).join("")}
    </select></div>
    ...
    <div class="form-group" data-standard-type-row><label>Unusual city type</label><select name="standardCode">
      ${Xh.map((e) => `<option value="${e.code}"${a < e.minimumTechLevel ? " disabled" : ""}>${og(`${e.code} — ${e.name} — minimum TL${e.minimumTechLevel}${a < e.minimumTechLevel ? " — unavailable at current TL" : ""}`)}</option>`).join("")}
    </select></div>
...
async function hg(e, t) {
	...
	let i = { ...cg(e) }, a = ig(n.cityAction) ?? "standard";
	if (a === "clear") delete i[String(r)];
	else if (a === "custom") {
		let e = ig(n.customCode)?.trim(), a = ig(n.customDescription)?.trim();
		...
		let a = ig(n.standardCode), o = ag(ug(e)?.social?.techLevel) ?? 0, s = Xh.find((e) => e.code === a);
		if (!a || !s) {
	...
	try {
		await e.update({ [`flags.${ng}.cityOverrides`]: i }), e.sheet?.render(!0), t.notifyInfo(a === "clear" ? `Cleared the unusual-city designation from Major City ${r}.` : `Updated the unusual-city designation for Major City ${r}.`);
	} catch (e) {
...
function bg(e, t) {
	return `<div style="display:grid;grid-template-columns:minmax(10rem,0.9fr) minmax(0,1.3fr);gap:0.75rem;padding:0.28rem 0;border-bottom:1px solid var(--color-border-light-primary);"><strong>${yg(e)}</strong><span style="min-width:0;overflow-wrap:anywhere;">${yg(t ?? "—")}</span></div>`;
}
function xg(e) {
	let t = th(e).find((e) => e.isMainworld), n = t?.social, r = t?.physical, i = Yh(gg(n?.majorCities));
	if (!t || !n || !i) return "<p class=\"hint\">No generated major cities are available for unusual-city designation.</p>";
	let a = _g(n.techLevel) ?? 0, o = _g(r?.atmosphereCode) ?? 0, s = Array.isArray(n.tradeCodes) ? n.tradeCodes.map(String) : [], c = $h({
		starport: vg(n.starport),
		hasHighport: !1,
		atmosphereCode: o,
		techLevel: a,
		tradeCodes: s
	}), l = cg(e), u = c.dmTotal >= 0 ? `+${c.dmTotal}` : String(c.dmTotal), d = i.cities.map((e) => {
		let t = l[String(e.rank)], n = tg(e.codes, e.rank, l), r = `${e.name}${n.length ? ` (${n.join(", ")})` : ""}: ${Math.trunc(e.population).toLocaleString("en-US")}: ${e.port ?? "—"}`, i = t ? `${r} — ${t.description}${t.source === "custom" ? " — custom Referee type" : ""}` : `${r} — normal city / no unusual designation`;
		return bg(`Major City ${e.rank}`, i);
	}).join(""), f = c.dmBreakdown.length ? `<div style="padding:0.35rem 0;"><strong>Optional occurrence modifiers</strong><ul style="margin:0.3rem 0 0 1.25rem;">${c.dmBreakdown.map((e) => `<li>${yg(`${e.source}: DM${e.dm >= 0 ? "+" : ""}${e.dm}`)}</li>`).join("")}</ul></div>` : "<p class=\"hint\">No optional unusual-city occurrence DMs apply.</p>";
	return [
		bg("World Tech Level", a),
		bg("Optional occurrence check", `2D 12+; world DM ${u}; natural roll ${c.effectiveNaturalRollRequired}+ required`),
		bg("Automatic assignment", "None — Referee controlled"),
		bg("Highport modifier", "Not applied — no highport has been established by TSG yet"),
		f,
...
function Tg(e, t) {
	let n = Sg(t.secondaryPopulationCode);
	...
	if (t.affiliation === "dependent") {
		...
		let r = Math.max(0, Math.trunc(t.ownerGovernmentCode)), i = Sg(t.ownerPopulationCode ?? 0);
		...
		let a = e.die(6), o = wg(r, i), s = a + o, c = Cg(s);
		return {
...
function Og(e, t) {
	let n = Math.max(0, Math.trunc(t.secondaryGovernmentCode)), r = t.affiliation === "dependent", i = Number.isFinite(t.ownerGovernmentCode) ? Math.max(0, Math.trunc(t.ownerGovernmentCode)) : null, a = Number.isFinite(t.ownerLawLevelCode) ? Eg(t.ownerLawLevelCode) : null, o = t.penalColony === !0, s = t.militaryBase === !0, c = t.freeport === !0;
	if (r && n === 6) {
		...
		} else u <= 4 ? p = a : u === 5 ? p = Eg(a + 1) : (d = e.die(6), p = Eg(a + d));
		return {
...
var kg = "traveller-system-generator";
function Ag(e) {
	return e && typeof e == "object" && !Array.isArray(e) ? e : null;
}
function jg(e) {
...
function Ig(e) {
	return Ag(e.flags?.[kg]);
}
function Lg(e) {
	let t = Ag(Ig(e)?.secondaryGovernmentOverrides);
	if (!t) return {};
	let n = {};
	for (let [e, r] of Object.entries(t)) {
		let t = Ag(r), i = Ag(t?.details), a = Ag(t?.lawDetails), o = Ag(t?.lawModifiers), s = t?.affiliation === "dependent" || t?.affiliation === "independent" ? t.affiliation : null;
		!t || !i || !s || (n[e] = {
			bodyId: e,
			affiliation: s,
			ownerId: Mg(t.ownerId),
			details: i,
			lawDetails: a,
			lawModifiers: {
...
function Rg(e) {
	return th(e).filter((e) => !e.isMainworld && e.social && (jg(e.social.populationCode) ?? 0) > 0);
}
function zg(e) {
	return th(e).find((e) => e.isMainworld && e.social);
}
function Bg(e, t) {
	let n = Ag(e.lawLevelDetails), r = Array.isArray(n?.balkanisedFactions) ? n.balkanisedFactions : [];
	for (let e of r) {
		let n = Ag(e), r = Ag(n?.details);
		if (n?.factionId === t) return jg(r?.lawLevelCode);
	}
	return null;
}
function Vg(e) {
	let t = zg(e);
	if (!t?.social) return [];
	let n = jg(t.social.populationCode) ?? 0, r = jg(t.social.governmentCode) ?? 0, i = jg(t.social.lawLevelCode) ?? 0, a = [{
		id: t.id,
		label: `${t.label} — Government ${r}, Law ${i}`,
		governmentCode: r,
		populationCode: n,
		lawLevelCode: i
	}], o = Ag(t.social.balkanisation), s = Array.isArray(o?.factions) ? o.factions : [];
	for (let e of s) {
		let r = Ag(e), i = Mg(r?.id), o = jg(r?.governmentCode);
...
function Hg(e) {
	return e.type === "world" && Rg(e).length > 0 && Vg(e).length > 0;
}
function Ug(e) {
	let t = Rg(e), n = Vg(e), r = Lg(e);
	return `
    ...
    <div class="form-group"><label>Secondary world</label><select name="bodyId">
      ${t.map((e) => {
		let t = r[e.id], n = jg(e.social?.populationCode) ?? 0, i = t?.lawDetails?.lawLevelCode;
		return `<option value="${Pg(e.id)}">${Pg(`${e.label} — Population ${n}${t ? ` — ${t.affiliation}, Government ${t.details.governmentCode}${i == null ? "" : `, Law ${i}`}` : ""}`)}</option>`;
	}).join("")}
    </select></div>
    ...
    <div class="form-group" data-owner-row><label>Owning authority</label><select name="ownerId">
      ${n.map((e) => `<option value="${Pg(e.id)}">${Pg(e.label)}</option>`).join("")}
    </select></div>
...
async function Gg(e, t) {
	...
	let n = await t.prompt({
		...
		rejectClose: !1,
		modal: !1,
		render: (t, n) => Wg(n.element, e)
	});
	if (!n) return;
	let r = Mg(n.bodyId), i = Rg(e).find((e) => e.id === r);
	if (!r || !i?.social) {
		t.notifyWarning("Select an inhabited secondary world.");
		return;
	}
	let a = { ...Lg(e) }, o = Mg(n.affiliation);
	if (o === "clear") delete a[r];
	else if (o === "dependent" || o === "independent") {
		let s = jg(i.social.populationCode) ?? 0, c;
		if (o === "dependent") {
			let r = Mg(n.ownerId);
			if (c = Vg(e).find((e) => e.id === r), !c) {
				t.notifyWarning("Select a valid owning authority for this dependency.");
				return;
			}
		}
		let l = jg(Ig(e)?.generationSeed) ?? 1, u = Tg(new S(Fg(`wbh-secondary-government-affiliation-v1|${l}|${r}|${o}|${c?.id ?? "independent"}`)), {
			affiliation: o,
			...
			penalColony: Ng(n.penalColony),
			militaryBase: Ng(n.militaryBase),
			freeport: Ng(n.freeport)
		}, f = Og(new S(Fg(`wbh-secondary-law-level-v1|${l}|${r}|${o}|${c?.id ?? "independent"}|${u.governmentCode}|${+!!d.penalColony}${+!!d.militaryBase}${+!!d.freeport}`)), {
			affiliation: o,
	...
	try {
		await e.update({ [`flags.${kg}.secondaryGovernmentOverrides`]: a }), e.sheet?.render(!0), t.notifyInfo(o === "clear" ? `Cleared the Referee secondary-government assignment for ${r}.` : `Updated the secondary-government and Law assignment for ${r}.`);
	} catch (e) {
...
function qg(e, t) {
	return `<div style="display:grid;grid-template-columns:minmax(10rem,0.9fr) minmax(0,1.3fr);gap:0.75rem;padding:0.28rem 0;border-bottom:1px solid var(--color-border-light-primary);"><strong>${Kg(e)}</strong><span style="min-width:0;overflow-wrap:anywhere;">${Kg(t ?? "—")}</span></div>`;
}
function Jg(e) {
	let t = Lg(e), n = th(e).filter((e) => !e.isMainworld && e.social && Number(e.social.populationCode ?? 0) > 0);
	return n.length ? [qg("WBH policy", "Referee chooses dependency or independence for each inhabited secondary world; Government and overall Law Level then follow the WBH secondary-world cases."), ...n.map((e) => {
		let n = t[e.id];
		if (!n) return qg(e.id, "No Referee secondary-government assignment");
		...
		return qg(e.id, `${n.affiliation} — owner ${i} — Government ${r.governmentCode} — ${a} — ${s}${r.independentGovernmentSixReview ? " — REVIEW: independent Government 6" : ""}`);
...
function Q(e, t) {
	return `<div style="display:grid;grid-template-columns:minmax(10rem,0.9fr) minmax(0,1.3fr);gap:0.75rem;padding:0.28rem 0;border-bottom:1px solid var(--color-border-light-primary);"><strong>${Xg(e)}</strong><span style="min-width:0;overflow-wrap:anywhere;">${Xg(t ?? "—")}</span></div>`;
}
function $(e, t, n = !1) {
	return `<details${n ? " open" : ""} style="margin-top:0.5rem;padding:0.5rem;border:1px dashed var(--color-border-light-primary);border-radius:4px;"><summary style="cursor:pointer;font-weight:600;">${Xg(e)}</summary><div style="margin-top:0.4rem;">${t}</div></details>`;
}
function Zg(e) {
	let t = Yg(e).flatMap((e) => {
		let t = Z(e);
		if (!t) return [];
...
function e_(e) {
	...
	return t ? [
		...
		$g("Energy", t.energy),
		$g("Electronics", t.electronics),
		$g("Manufacturing", t.manufacturing),
		$g("Medical", t.medical),
		$g("Environmental", t.environmental)
...
function n_(e) {
	...
	return t ? [
		...
		$g("Personal Military", t.personal),
		$g("Heavy Military", t.heavy)
...
function r_(e) {
	...
	let n = Yg(t.factors).map(Z).filter((e) => !!e);
	return [
		Q("Status", t.status ?? "—"),
		...
		Q("Unresolved factors", Yg(t.unresolvedFactors).join(", ") || "None"),
...
function i_(e) {
	...
	return [
		...
		Q("Novelty", n?.novelty ?? "—"),
		Q("Provisional reasons", Yg(t.provisionalReasons).join("; ") || "None")
	].join("");
}
function a_(e) {
	...
	return [
		...
		Q("Low Common TLM", Qg(t.lowCommonTlm)),
		Q("Low Common DMs", Zg(t.lowCommonDmBreakdown)),
...
function o_(e) {
	...
	if (!t) return "";
	let n = Yg(t.factions).map(Z).filter((e) => !!e);
	return $("Government-7 Common TL candidates", [
		...
		...n.map((e) => {
			...
			return $(`Faction ${e.factionId ?? "?"}`, [
				...
				Q("Non-host High TLM", Qg(n?.highCommonTlm)),
				Q("Non-host High DMs", Zg(n?.highCommonDmBreakdown)),
				Q("Non-host Low Common TL", n?.lowCommonTechLevel ?? "Incomplete — faction PCR not established"),
				Q("Non-host Low DMs", Zg(n?.lowCommonDmBreakdown))
			].join(""));
		})
	].join(""));
}
function s_(e, t, n) {
	let r = Z(e);
	if (!r) return `<p class="hint">${Xg(n)}</p>`;
	let i = Z(r.world);
	if (i) return t(i);
	let a = Yg(r.factions).map(Z).filter((e) => !!e);
	return a.length ? a.map((e) => $(`Faction ${e.factionId ?? "?"} candidates`, [$("Starport host / adjacent", t(e.hostOrAdjacent), !0), $("Non-host", t(e.nonHost), !1)].join(""))).join("") : `<p class="hint">${Xg(n)}</p>`;
}
function c_(e) {
	let t = Z(e);
	return t ? t.populationCode === 0 ? "<p class=\"hint\">Population 0: no inhabited-world Technology profile is generated.</p>" : [
		$("Technology Profile", s_(t.technologyProfileDetails, i_, "No final WBH Technology Profile is stored."), !0),
		$("Common Technology", a_(t), !0),
		o_(t),
		$("Quality of Life Technology", s_(t.qualityOfLifeTechnologyDetails, e_, "No WBH Quality of Life Technology details are stored."), !0),
		$("Transportation Technology", s_(t.transportationTechnologyDetails, t_, "No WBH Transportation Technology details are stored."), !0),
		$("Military Technology", s_(t.militaryTechnologyDetails, n_, "No WBH Military Technology details are stored."), !0),
		$("Novelty Technology", s_(t.noveltyTechnologyDetails, r_, "No WBH Novelty Technology details are stored."), !0),
...
var l_ = "traveller-system-generator";
function u_(e) {
	return e && typeof e == "object" && !Array.isArray(e) ? e : null;
}
function d_(e) {
...
function p_(e) {
	return u_(u_(e.flags?.[l_])?.sensorSystem)?.secondaryPopulationPlan ?? null;
}
function m_(e, t) {
	let n = e.indexOf("<div data-wbh-selected-body>");
	if (n < 0) return e;
	let r = [
		f_("Secondary World Populations", Kh(p_(t))),
		f_("Secondary World Governments", Jg(t)),
		f_("Unusual Cities", xg(t))
	].join("\n    ");
...
function h_(e, t) {
	...
	let i = th(t).find((e) => e.id === n.value);
	if (!i) return;
	let a = document.createElement("div");
	a.dataset.wbhTechnologySection = "true", a.innerHTML = f_("Technology", c_(i.social));
...
function g_(e, t) {
	...
	n && n.addEventListener("change", () => h_(e, t));
}
async function __(e, t, n) {
	await Dh(e, {
		...
		prompt: async (n) => t.prompt({
			...n,
			content: m_(n.content, e),
			render: (t, r) => {
				n.render?.(t, r), g_(r.element, e);
			}
		})
	}, n);
}
...
function y_(e, t, n) {
	e.push({
		...
		onClick: (e, r) => {
			...
			i && Jm(t, n, {
				initialActorId: i.id,
				initialAction: im(i) ? "adopt" : "update"
			});
		}
	});
}
...
var x_ = "traveller-system-generator", S_ = "useDefaultSystemFolder", C_ = "defaultSystemFolderId";
function w_(e, t) {
	return Object.fromEntries([["", t.localize("TSG.Dialog.ActorFolderRoot")], ...Wp(e).map((e) => [e.id, e.path])]);
}
function T_(e, t) {
	e.register(x_, S_, {
...
async function E_(e) {
	let t = w_(e.listActorFolders(), e.localizer), n = e.settings.settings.get(`${x_}.${C_}`);
	n && (n.choices = t);
	let r = String(e.settings.get("traveller-system-generator", "defaultSystemFolderId") ?? "");
	r && !(r in t) && (await e.settings.set(x_, C_, ""), e.settings.get("traveller-system-generator", "useDefaultSystemFolder") && e.notifyWarning(e.localizer.localize("TSG.Notification.DefaultFolderMissing")));
}
function D_(e, t) {
...
function O_(e) {
	...
	e.Hooks.once("init", () => {
		...
		T_(n.settings, n.localizer);
	}), e.Hooks.once("ready", () => {
		E_(e.getDefaultFolderSettingsEnvironment());
	});
	for (let t of [
		"createFolder",
		"updateFolder",
	...
	e.Hooks.on("getSceneControlButtons", (n) => {
		Ym(n, e.getManagerEnvironment(), t);
...
function j_(e, t) {
	let n = new Map(t.map((e) => [e.id, e])), r = [], i = /* @__PURE__ */ new Set(), a = k_(e.folder);
	for (; a && !i.has(a);) {
		i.add(a);
		let e = n.get(a);
		if (!e) break;
		r.unshift(e.name), a = k_(e.folder ?? e.parent);
	}
	return r.join(" / ");
}
if (typeof Hooks < "u") {
	...
	}, t = (e) => foundry.applications.api.DialogV2.input(e), n = () => A_(Array.from(game.folders));
	O_({
		Hooks,
...
//#endregion
export { O_ as registerFoundryBootstrap };
