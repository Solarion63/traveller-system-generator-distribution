...
function y(e) {
	let t = Math.floor(e), n = Math.ceil(e), r = v.get(t), i = v.get(n);
	if (r === void 0) throw Error(`Orbit# ${t} is below supported range.`);
	if (i === void 0) return v.get(18) * 2 ** (e - 18);
	if (t === n) return r;
	let a = e - t;
	return r + (i - r) * a;
}
function te(e) {
	let t = [...v.entries()].sort((e, t) => e[0] - t[0]);
...
function x(e, t = 0) {
	...
	return r = n <= 5 ? Math.max(0, -.001 + e.d6() / 1e3) : n <= 7 ? e.d6() / 200 : n <= 9 ? .03 + e.d6() / 100 : n === 10 ? .05 + e.d6() / 20 : n === 11 ? .05 + e.roll(2, 6).total / 20 : .3 + e.roll(2, 6).total / 20, b(Math.max(0, Math.min(.999, r)), 3);
}
//#endregion
//#region src/rules/stars/starTables.ts
function ne(e, t = "classic") {
...
function S(e, t, n, r) {
	let i = se.filter((t) => t.spectralType === e);
...
function de(e, t, n, r, i) {
	if (r === "BD") {
		...
		return {
			...
			diameterSolar: s,
			temperatureK: o,
			luminositySolar: Number(C(s, o).toFixed(6)),
	...
	if (r === "D") {
		...
		return {
			...
			temperatureK: r,
			luminositySolar: Number(C(o, r).toFixed(6)),
...
function fe(e, t, n) {
	let r = ne(e.roll(2, 6, t ? 0 : -1).total, n);
	return r === "Hot" ? ae(e.roll(2, 6).total) : r;
}
function pe(e, t, n, r, i) {
	let a = le(i), o = fe(e, r, a.starDistribution), s = "V", c = "";
	if (o === "Special") {
		let i = re(e.roll(2, 6).total, a.allowUnusualPrimaries);
		if (i === "BD" || i === "D" || i === "Peculiar") return de(e, t, n, i, r);
		s = i === "III" ? ie(e.roll(2, 6).total) : i, c = `Special-system result resolved as luminosity class ${s}.`;
	}
	let l;
	if (o === "Special") {
		let t = ne(e.roll(2, 6, 1).total, a.starDistribution);
		l = t === "Hot" ? ae(e.roll(2, 6).total) : t === "Special" ? "M" : t;
	} else l = o;
	let u = oe(e.roll(2, 6).total, l, r), d = ue(l, u, s);
	l = d.spectralType, u = d.subtype;
	let f = S(l, u, s, "mass"), p = S(l, u, s, "diameter"), m = S(l, u, s, "temperatureK"), h = C(p, m), g = s === "V" || s === "VI" ? w(e, f) : Math.max(w(e, Math.max(.9, Math.min(f, 3))), 1);
	return {
		id: n,
		name: t,
		designation: n,
...
function Ee(e, t) {
	return b(Math.sqrt(e ** 3 / Math.max(t, .01)), 3);
}
function De(e, t, n) {
...
function Ye(e, t, n, r) {
	...
	for (let o of [
		...
		n.orbitClass = o.orbitClass, n.orbitNumber = b(Je(e, o.orbitClass), 2), n.orbitAu = b(y(n.orbitNumber), 3), n.eccentricity = x(e, 2), i.push(n);
	}
	if (r.detailLevel !== "basic") {
		...
		for (let o of n) if (e.roll(2, 6, a - (r.detailLevel === "deep" ? 0 : 1)).total >= 10) {
			...
			a.orbitClass = "Companion", a.parentId = o.id, a.orbitNumber = b(Je(e, "Companion"), 2), a.orbitAu = b(y(a.orbitNumber), 3), a.eccentricity = x(e, 2), i.push(a);
		}
	}
	return i;
}
function Xe(e) {
	...
	return b(Math.max(-2, te(t)), 2);
}
function Ze(e) {
	return b(te(Math.sqrt(Math.max(e.luminositySolar, 1e-6))), 2);
}
function Qe(e) {
	let t = [];
	for (let n of e) {
		...
		t.push({
			...
			inner: b(n.orbitNumber - e, 2),
			outer: b(n.orbitNumber + e, 2),
...
function nt(e, t, n, r, i) {
	let a = Math.max(1, n.totalWorlds), o = tt(e, i.detailLevel), s = a + Math.max(0, o), c = et(e, a), l = t[0], u = l?.hzco ?? 4, d = l?.minimumAllowableOrbit ?? -2, f = b(Math.max(.1, (u - d) / c), 2), p = {
		"Empty Orbit": o,
	...
	for (let n of t) {
		...
		for (; t <= n.allocatedWorlds && i < n.allocatedWorlds * 8 + 16;) {
			i += 1;
			let a = t - c, o = e.roll(2, 6, -7).total * .05 * f, s = b(Math.max(n.minimumAllowableOrbit, n.hzco + a * f + o), 2), l = j(s, n.star.id, r), u = [];
			if (l && (s = b(l.outer + .2 + e.d10ZeroToNine() / 20, 2), u.push(`Moved outward to avoid forbidden zone from ${l.source}.`)), s > n.outerLimit && (s = b(n.outerLimit - e.d10ZeroToNine() / 10, 2)), j(s, n.star.id, r)) continue;
			let d = b(y(s), 3), h = b(s - n.hzco, 2), g = x(e, s < 1 ? -1 : 0), _ = {
				orbitNumber: s,
				hzDeviation: h
			};
			m.push({
	...
	for (; m.length < s;) {
		let n = t[0], r = m.filter((e) => e.aroundStarId === n.star.id).length + 1, i = b(n.hzco + r * f, 2), a = b(i - n.hzco, 2);
		m.push({
...
function N(e = {}) {
	let t = Ke(e.settings), n = e.seed ?? Date.now(), r = new ee(rt(n, e.rerollIndex ?? 0)), i = e.name ?? "Uncharted System", a = T(r, `${i} A`, "A", !0, t), o = Ye(r, i, a, t).sort((e, t) => (e.orbitAu ?? 0) - (t.orbitAu ?? 0)), s = ge(r, o.length), c = Qe(o), l = o.filter((e) => e.orbitClass !== "Companion"), u = t.detailLevel === "deep" ? 4 : t.detailLevel === "basic" ? -2 : 0, d = nt(r, $e(l.map((e) => {
		let t = Xe(e), n = Ze(e), r = e.orbitClass === "Primary" ? 18 + u : Math.max(2, (e.orbitNumber ?? 10) - 1), i = c.filter((t) => t.aroundStarId === e.id).sort((e, t) => e.inner - t.inner)[0], a = Math.max(t + 1, Math.min(r, i ? i.inner - .2 : r));
		return {
			star: e,
...
var it = /^([ABCDEX])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])([0-9A-HJ-NP-Z])-([0-9A-HJ-NP-Z])$/i;
function at(e) {
	let t = e.trim().toUpperCase().replace(/\s+/g, "").match(it);
...
function st(e, t) {
	let n = e.primary, r = Math.round((e.worlds.find((e) => e.aroundStarId === n.id)?.hzco ?? 4) * 100) / 100, i = b(r + t.roll(2, 6, -7).total * .04, 2), a = b(y(i), 3);
	return {
		id: "A-MW",
		aroundStarId: n.id,
...
function lt(e) {
	let t = at(e.sourceUwp), n = N(e), r = new ee(ot(e.seed ?? Date.now(), (e.rerollIndex ?? 0) + 101)), i = st(n, r), a = ke(i, n.primary, t.size, t.atmosphere, t.hydrographics), o = t.population === 0 ? 0 : r.die(9), s = Ue(t.starport, t.population, o, t.government, t.law, t.techLevel, t.size, t.atmosphere, t.hydrographics), c = n.worlds.filter((e) => !e.isMainworld).map((e) => ({
		...e,
		isMainworld: !1
	})).sort((e, t) => e.au - t.au).map(ct), l = [{
		...i,
		physical: a,
		social: s
...
var P = "traveller-system-generator";
function dt() {
	return {
		moduleId: P,
		generateSystem(e = {}) {
			return e.method === "continuation" ? lt(e) : N(e);
		},
		createTwodsixWorldActor(e) {
			let t = _(e), n = t.flags[P];
			return t.flags[P] = {
				...n && typeof n == "object" ? n : {},
				sensorSystem: ut(e)
			}, t;
...
function ft(e, t = dt()) {
	let n = e.get(P);
	if (!n) throw Error(`Foundry module ${P} is not registered.`);
	return n.api = t, t;
}
//#endregion
...
var pt = {
	...
	format(e, t) {
		return Object.entries(t).reduce((e, [t, n]) => e.replaceAll(`{${t}}`, String(n)), pt[e]);
	}
};
//#endregion
...
function R(e) {
	...
	for (let e of n.values()) e.sort(ht);
	...
	for (let t of [...e].sort(ht)) i.has(t.id) || a(t, 0, []);
	return r;
}
function gt(e, t, n = "") {
	let r = R(e).map((e) => {
		let t = e.depth > 0 ? `${"\xA0\xA0".repeat(e.depth)}└─ ` : "", r = e.id === n ? " selected" : "";
		return `<option value="${F(e.id)}"${r}>${F(`${t}${e.path}`)}</option>`;
	});
	return [`<option value=""${n ? "" : " selected"}>${F(t.localize("TSG.Dialog.ActorFolderRoot"))}</option>`, ...r].join("");
}
function _t(e = 1105, t = [], n = mt, r = "") {
	let i = n.localize;
	return `
    <div class="form-group"><label>${i("TSG.Dialog.SystemName")}</label><input name="name" type="text" value="${F(i("TSG.Dialog.DefaultSystemName"))}" autofocus></div>
    <div class="form-group"><label>${i("TSG.Dialog.Seed")}</label><input name="seed" type="number" value="${F(String(e))}" step="1"></div>
    ...
    <div class="form-group"><label>${i("TSG.Dialog.ActorFolder")}</label><select name="folder">${gt(t, n, r)}</select></div>
...
function bt(e) {
	let t = z(e.name, "Uncharted System").trim() || "Uncharted System", n = vt(e.seed, 1105), r = z(e.method, "expanded"), i = {
		name: t,
		seed: n,
		settings: {
			starDistribution: z(e.starDistribution, "classic") === "realistic" ? "realistic" : "classic",
			detailLevel: ["basic", "deep"].includes(z(e.detailLevel)) ? z(e.detailLevel) : "standard",
			allowUnusualPrimaries: yt(e.allowUnusualPrimaries, !0)
		}
	};
	return {
		request: r === "continuation" ? {
			...i,
			method: "continuation",
			sourceUwp: z(e.sourceUwp, "A867A74-C").trim().toUpperCase()
		} : {
			...i,
			method: "expanded"
		},
		folder: z(e.folder).trim() || null,
		openSheet: yt(e.openSheet, !0)
	};
}
function xt(e) {
	return bt(e).request;
...
async function St(e, t) {
	...
	let r = e.listActorFolders(), i = e.defaultSystemFolderId?.() ?? "", a = await e.prompt({
		...
		rejectClose: !1,
		modal: !1,
		render: (e, t) => I(t.element)
	});
	if (a) try {
		let r = bt(a), i = t.generateSystem(r.request), o = t.createTwodsixWorldActor(i), s = o.flags && typeof o.flags == "object" ? o.flags : {};
...
var B = "traveller-system-generator", Ct = [
	"notes",
	"adventureHooks",
	"relatedActors",
	"worldImage"
...
function U(e) {
	return !!(e && e.type === "world" && H(e));
}
function W(e) {
	return !!(e && e.type === "world" && !H(e));
}
function wt(e, t, { adopting: n = !1 } = {}) {
	let r = H(e) ?? {}, i = r.generationSettings ?? {}, a = String(e.system.name ?? e.name.replace(/ System$/, "")), o = r.generationMethod ?? "expanded", s = r.sourceUwp ?? String(e.system.uwp ?? "A867A74-C"), c = r.generationSeed ?? (n ? L() : 1105), l = i.starDistribution ?? "classic", u = i.detailLevel ?? "standard", d = i.allowUnusualPrimaries ?? !0, f = t.localize;
	return `
    <p class="hint">${V(f(n ? "TSG.Dialog.AdoptHint" : "TSG.Dialog.RegenerateHint"))}</p>
    <div class="form-group"><label>${f("TSG.Dialog.SystemName")}</label><input name="name" type="text" value="${V(a)}" autofocus></div>
    ...
    <div class="form-group"><label>${f("TSG.Dialog.SourceUwp")}</label><input name="sourceUwp" type="text" value="${V(s)}" maxlength="9"><p class="hint">${f("TSG.Dialog.SourceUwpHint")}</p></div>
...
function Et(e, t) {
	return wt(e, t, { adopting: !0 });
}
function Dt(e, t, n) {
	let r = { ...t.system };
	for (let t of Ct) e.system[t] !== void 0 && (r[t] = e.system[t]);
	let i = { ...t.flags };
	return i[B] = {
		...i[B] ?? {},
		generationSeed: n
...
async function Ot(e, t, n, r) {
	let i = xt(t), a = r.generateSystem(i), o = r.createTwodsixWorldActor(a);
	await e.update(Dt(e, o, Number(i.seed ?? 1105))), t.openSheet !== !1 && e.sheet?.render(!0);
}
async function kt(e, t) {
	...
	if (r) try {
		await Ot(n, r, e, t), e.notifyInfo(e.localizer.format("TSG.Notification.Regenerated", { name: n.name }));
	} catch (t) {
...
async function At(e, t) {
	...
	if (r) try {
		await Ot(n, r, e, t), e.notifyInfo(e.localizer.format("TSG.Notification.Adopted", { name: n.name }));
	} catch (t) {
		let n = t instanceof Error ? t.message : String(t);
		e.notifyError(e.localizer.format("TSG.Notification.Failed", { message: n }));
	}
...
var jt = "traveller-system-generator", G = "__all__", K = "__root__";
function q(e) {
	return e.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\"/g, "&quot;").replace(/'/g, "&#039;");
}
function Mt(e) {
...
function Nt(e, t = "", n = G) {
	return J(e).filter((e) => n === G || (n === K ? !e.folderId : e.folderId === n)).map((e) => {
		let n = U(e), r = `${e.name}${n ? "" : " — not generator-managed"}`;
		return `<option value="${q(e.id)}" data-folder-id="${q(e.folderId ?? "")}" data-generator-managed="${n ? "true" : "false"}"${e.id === t ? " selected" : ""}>${q(r)}</option>`;
	}).join("");
}
function Pt(e, t = G) {
	let n = e.localizer.localize("TSG.Dialog.ActorFolderRoot"), r = R(e.listActorFolders());
	return [
		`<option value="${G}"${t === G ? " selected" : ""}>All Actor folders</option>`,
		`<option value="${K}"${t === K ? " selected" : ""}>${q(n)}</option>`,
		...r.map((e) => `<option value="${q(e.id)}"${e.id === t ? " selected" : ""}>${q(e.path)}</option>`)
	].join("");
}
function Ft(e) {
	if (!e) return null;
	let t = e.flags?.[jt], n = t && typeof t == "object" ? t : {}, r = n.generationSettings, i = r && typeof r == "object" ? r : {}, a = U(e);
	return {
...
function Y(e, t) {
	return `<div style="display:grid;grid-template-columns:minmax(8rem,0.8fr) minmax(0,1.2fr);gap:0.75rem;padding:0.3rem 0;border-bottom:1px solid var(--color-border-light-primary);"><strong>${q(e)}</strong><span style="min-width:0;overflow-wrap:anywhere;">${q(t)}</span></div>`;
}
function It(e, t) {
	let n = t.localizer.localize, r = Ft(e);
    ...
    <div style="padding:0.75rem;border:1px solid var(--color-border-light-primary);border-radius:6px;background:var(--color-bg-option);min-width:0;">
      <h3 style="margin:0 0 0.5rem;">${q(e.name)}</h3>
      ${Y("Generator status", r.managed ? "Generator-managed" : "Not generator-managed")}
      ${Y(n("TSG.Manager.Folder"), r.folderPath)}
      ${Y(n("TSG.Manager.Uwp"), r.uwp)}
      ${Y(n("TSG.Manager.Method"), r.method)}
      ${Y(n("TSG.Manager.Seed"), r.seed)}
      ${Y(n("TSG.Manager.Distribution"), r.distribution)}
      ${Y(n("TSG.Manager.DetailLevel"), r.detailLevel)}
      ${r.managed ? "" : `<p class="hint" style="margin:0.6rem 0 0;">${q(n("TSG.Manager.AdoptHint"))}</p>`}
    </div>`;
}
function X(e, t, n, r, i = !1) {
    ...
    <label style="display:block;padding:0.75rem;border:1px solid var(--color-border-light-primary);border-radius:6px;cursor:${i ? "not-allowed" : "pointer"};opacity:${i ? "0.55" : "1"};" data-manager-action-card="${e}">
      <span style="display:flex;align-items:flex-start;gap:0.6rem;">
        ...
        <span><strong>${q(t)}</strong><br><span class="hint">${q(n)}</span></span>
      </span>
    </label>`;
}
function Lt(e) {
	return e?.folderId || (e ? K : G);
}
function Rt(e, t, n = {}) {
	let r = t.localizer.localize, i = J(e), a = i.find((e) => e.id === n.initialActorId) ?? i[0], o = n.initialActorId ? Lt(a) : G, s = o === G ? i : i.filter((e) => o === K ? !e.folderId : e.folderId === o), c = s.find((e) => e.id === a?.id) ?? s[0], l = Nt(i, c?.id, o), u = n.initialAction ?? "generate", d = i.length > 0, f = !!(c && U(c)), p = !!(c && W(c));
	return `
    <section style="display:grid;gap:0.8rem;min-width:0;">
      ...
      ${X("generate", r("TSG.Manager.Generate"), r("TSG.Manager.GenerateDescription"), u === "generate")}
    </section>
    ...
    <section style="display:grid;gap:0.8rem;min-width:0;">
      ...
      <div class="form-group" style="min-width:0;"><label>Actor Folder</label><select name="managerFolderId" aria-label="Actor Folder" style="width:100%;min-width:0;max-width:100%;">${Pt(t, o)}</select></div>
      ...
      <div data-manager-actor-details style="min-width:0;">${It(c, t)}</div>
      <div style="display:grid;grid-template-columns:1fr;gap:0.75rem;">
        ${X("open", r("TSG.Manager.Open"), r("TSG.Manager.OpenDescription"), u === "open", !d)}
        ${X("update", r("TSG.Manager.Update"), r("TSG.Manager.UpdateDescription"), u === "update" && f, !f)}
        ${X("adopt", r("TSG.Manager.Adopt"), r("TSG.Manager.AdoptDescription"), u === "adopt" && p, !p)}
      </div>
    </section>`;
}
function zt(e) {
...
function Bt(e, t, n) {
	...
	i.addEventListener("change", () => {
		let e = i.value || G, t = a.value, n = Nt(r, t, e);
...
async function Ht(e, t, n = {}) {
	...
	let r = e.listGeneratedActors(), i = await e.prompt({
		...
		rejectClose: !1,
		modal: !1,
		render: (t, n) => Bt(n.element, r, e)
	});
	if (!i) return;
	let a = zt(i.action) || n.initialAction || "generate";
	if (a === "generate") {
		await St(e, t);
		return;
	}
	let o = J(r).find((e) => e.id === zt(i.actorId));
	if (!o) {
...
function Ut(e, t, n) {
	e.push({
		...
		callback: (e) => {
			let r = Mt(e), i = t.listGeneratedActors().find((e) => e.id === r);
			Ht(t, n, {
				initialActorId: r,
				initialAction: i && W(i) ? "adopt" : "update"
			});
		}
	});
}
...
var Z = "traveller-system-generator", Gt = "useDefaultSystemFolder", Q = "defaultSystemFolderId";
function Kt(e, t) {
	return Object.fromEntries([["", t.localize("TSG.Dialog.ActorFolderRoot")], ...R(e).map((e) => [e.id, e.path])]);
}
function qt(e, t) {
	e.register(Z, Gt, {
...
async function Jt(e) {
	let t = Kt(e.listActorFolders(), e.localizer), n = e.settings.settings.get(`${Z}.${Q}`);
	n && (n.choices = t);
	let r = String(e.settings.get("traveller-system-generator", "defaultSystemFolderId") ?? "");
	r && !(r in t) && (await e.settings.set(Z, Q, ""), e.settings.get("traveller-system-generator", "useDefaultSystemFolder") && e.notifyWarning(e.localizer.localize("TSG.Notification.DefaultFolderMissing")));
}
function Yt(e, t) {
...
function Xt(e) {
	...
	e.Hooks.once("init", () => {
		...
		qt(n.settings, n.localizer);
	}), e.Hooks.once("ready", () => {
		Jt(e.getDefaultFolderSettingsEnvironment());
	});
	for (let t of [
		"createFolder",
		"updateFolder",
	...
	e.Hooks.on("getSceneControlButtons", (n) => {
		Wt(n, e.getManagerEnvironment(), t);
	}), e.Hooks.on("getActorContextOptions", (n, r) => {
		Ut(r, e.getManagerEnvironment(), t);
	});
}
function $(e) {
...
if (typeof Hooks < "u") {
	...
	}, t = (e) => foundry.applications.api.DialogV2.input(e), n = () => Zt(Array.from(game.folders));
	Xt({
		Hooks,
...
//#endregion
export { Xt as registerFoundryBootstrap };
